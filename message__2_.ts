import { getData, setData, React, Message, Channel, Dm } from './dataStore';
 ...
 * @param {Message} messageFull
 * @param {Channel} channel
 * @param {number} authUserId
 */
const sendDelayedMessage = (messageFull: Message, channel: Channel, authUserId: number) => {
  const data = getData();
  const user = data.users.find(u => u.uId === authUserId);
  const timeNow = Math.floor(new Date().getTime() / 1000);
  messageFull.timeSent = timeNow;

  channel.messages.unshift(messageFull);
  reservedMessages -= 1;

  // add notif
  const msg = messageFull.message.slice(0, 20);
  ...
  const handles = checkMsgTag(messageFull.message);
  for (const h of handles) {
    const user = data.users.find(u => u.handleStr === h);
    if (channel.allMembers.includes(user.uId)) {
      user.notifications.unshift(notif);
 ...
 * @param {number} authUserId
 */
const sendDelayedMessageDM = (messageFull: Message, dm: Dm, authUserId: number) => {
  const data = getData();
  const message = messageFull.message;
  const user = data.users.find(u => u.uId === authUserId);
  const timeNow = Math.floor(new Date().getTime() / 1000);
  messageFull.timeSent = timeNow;

  dm.messages.unshift(messageFull);
  reservedMessages -= 1;
...
export const messageSendLaterV1 = (token: string, channelId: number, message: string, timeSent: number) => {
  ...
  setTimeout(() => {
    ...
    sendDelayedMessage(lateMessage, channel, authUserId);
    setData(data);
...
export const messageSendLaterDMV1 = (token: string, dmId: number, message: string, timeSent: number) => {
  ...
  setTimeout(() => {
    ...
    sendDelayedMessageDM(lateMessage, dm, authUserId);
    setData(data);
  }, (timeSent - timeNow) * 1000);
